...
void main() {
  group('X-Wing', () {
    ...
    test('finds row-based X-Wing', () {
      ...
      board.getCell(0, 1).setCandidates({5, 6});
      board.getCell(0, 4).setCandidates({5, 7});
      // Row 3: 5 at cols 1 and 4 only.
      board.getCell(3, 1).setCandidates({5, 8});
      board.getCell(3, 4).setCandidates({5, 9});

      // Other rows with 5 in cols 1 or 4 → should be eliminated.
      board.getCell(1, 1).setCandidates({5, 3});
      board.getCell(6, 4).setCandidates({5, 2});
    ...
    test('finds column-based X-Wing', () {
      ...
      board.getCell(2, 0).setCandidates({3, 4});
      board.getCell(7, 0).setCandidates({3, 6});
      board.getCell(2, 5).setCandidates({3, 8});
      board.getCell(7, 5).setCandidates({3, 9});

      // Other columns with 3 in rows 2 or 7.
      board.getCell(2, 3).setCandidates({3, 7});
      board.getCell(7, 8).setCandidates({3, 1});
    ...
    test('returns null when no X-Wing exists', () {
      ...
      board.getCell(0, 4).setCandidates({5, 7});
      board.getCell(3, 1).setCandidates({5, 8}); // different column pattern
  ...
  group('Swordfish', () {
    ...
    test('finds row-based Swordfish', () {
      ...
      board.getCell(0, 1).setCandidates({7, 2});
      board.getCell(0, 4).setCandidates({7, 3});
      board.getCell(3, 4).setCandidates({7, 5});
      board.getCell(3, 7).setCandidates({7, 6});
      board.getCell(6, 1).setCandidates({7, 8});
      board.getCell(6, 7).setCandidates({7, 9});

      // Other rows with 7 in those columns.
      board.getCell(2, 1).setCandidates({7, 4});
      board.getCell(5, 4).setCandidates({7, 8});
      board.getCell(8, 7).setCandidates({7, 1});
  ...
  group('Jellyfish', () {
    ...
    test('finds row-based Jellyfish', () {
      ...
      board.getCell(0, 0).setCandidates({2, 4});
      board.getCell(0, 3).setCandidates({2, 6});
      board.getCell(2, 3).setCandidates({2, 8});
      board.getCell(2, 5).setCandidates({2, 9});
      board.getCell(5, 5).setCandidates({2, 1});
      board.getCell(5, 7).setCandidates({2, 3});
      board.getCell(8, 0).setCandidates({2, 7});
      board.getCell(8, 7).setCandidates({2, 5});

      // Elimination targets.
      board.getCell(1, 0).setCandidates({2, 6});
      board.getCell(4, 5).setCandidates({2, 8});
