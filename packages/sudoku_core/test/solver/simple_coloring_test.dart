...
void main() {
  final strategy = SimpleColoring();

  group('Simple Coloring', () {
    test('Rule 4: eliminates candidate that sees both colors', () {
      ...
      expect(step!.strategy, StrategyType.simpleColoring);
    ...
    test('Rule 2: color clash eliminates same-color cells', () {
      final board = Board.empty();

      // Build a chain where two cells of the same color are peers.
      ...
      // Actually, a 4-cell cycle with 2 colors: if there's an odd cycle,
      // two adjacent cells get the same color. This 4-cycle is even, so no clash.
