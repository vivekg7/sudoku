...
void main() {
  group('UniqueRectangle', () {
    ...
    test('Type 1: eliminates UR candidates from non-bi-value roof', () {
      ...
      board.getCell(0, 0).setCandidates({3, 7});
      board.getCell(0, 3).setCandidates({3, 7});
      board.getCell(1, 0).setCandidates({3, 7});
      board.getCell(1, 3).setCandidates({3, 7, 9});
    ...
    test('Type 2: eliminates extra candidate from common peers', () {
      ...
      board.getCell(0, 0).setCandidates({3, 7});
      board.getCell(0, 3).setCandidates({3, 7});
      board.getCell(1, 0).setCandidates({3, 7, 5});
      board.getCell(1, 3).setCandidates({3, 7, 5});

      // Cells in row 1 that see both roof cells and have candidate 5.
      board.getCell(1, 1).setCandidates({5, 8});
      board.getCell(1, 5).setCandidates({5, 9});
    ...
    test('Type 4: locked candidate eliminates other UR value from roof', () {
      ...
      board.getCell(0, 3).setCandidates({3, 7});
      board.getCell(1, 0).setCandidates({3, 7, 5});
      board.getCell(1, 3).setCandidates({3, 7, 8});

      // No other cell in row 1 has candidate 3.
      // Avoid extras {5,8} forming a naked pair with any cell (prevents Type 3).
      board.getCell(1, 1).setCandidates({4, 6});
      board.getCell(1, 2).setCandidates({6, 9});
      board.getCell(1, 4).setCandidates({4, 9});
      board.getCell(1, 5).setCandidates({2, 9});
      board.getCell(1, 6).setCandidates({2, 6});
      board.getCell(1, 7).setCandidates({4, 9});
      board.getCell(1, 8).setCandidates({2, 6});
    ...
    test('returns null when no UR pattern exists', () {
      final board = Board.empty();
      board.getCell(0, 0).setCandidates({3, 7});
      board.getCell(0, 3).setCandidates({3, 8}); // different candidates
